...
func LoginInfoAdd(param system.LoginParam, message string, loginSucess bool) R.Result {
	var status = "0"
	if loginSucess {
		status = "0"
	} else {
	...
	if err := db.Db().Model(SysLogininfor{}).Create(&info).First(&SysLogininfor{}).Error; err != nil {
...
func DelectLoginlog(operIds []int) R.Result {
	if err := db.Db().Model(&SysLogininfor{}).Delete("info_id in (?)", operIds).Error; err != nil {
		return R.ReturnFailMsg(err.Error())
	}
	return R.ReturnSuccess("操作成功")
}
...
func ClearLoginlog() R.Result {
	if err := db.Db().Model(&SysLogininfor{}).Raw("truncate table sys_logininfor").Find(SysLogininfor{}).Error; err != nil {
		return R.ReturnFailMsg(err.Error())
	}
	return R.ReturnSuccess("操作成功")
}
