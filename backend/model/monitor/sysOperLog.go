...
func DelectOperlog(operIds []int) R.Result {
	if err := db.Db().Where("oper_id in (?)", operIds).Delete(&SysOperLog{}).Error; err != nil {
...
func ClearOperlog() R.Result {
	if err := db.Db().Exec("truncate table sys_oper_log").Error; err != nil {
		return R.ReturnFailMsg(err.Error())
	}
	return R.ReturnSuccess("操作成功")
}

func (param *SysOperLog) OperationLogAdd() R.Result {
	...
	if err := db.Db().Model(&SysOperLog{}).Create(&param).Error; err != nil {
		return R.ReturnFailMsg(err.Error())
	}
	return R.ReturnSuccess("操作成功")
}
