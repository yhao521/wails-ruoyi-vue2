...
func SelectMenuTree(userId int, menu SysMenu) []SysMenu {
	...
	if IsAdminById(userId) {
		...
		err := db.Db().Raw(sql).Scan(&menus).Error
		if err != nil {
...
func SelectSysMenuList(params tools.SearchTableDataParam) []SysMenu {
	...
	err := db.Db().Raw(sql).Find(&rows).Error
	if err != nil {
		panic(R.ReturnFailMsg(err.Error()))
	}
	return rows
...
func SelectMenuListByRoleId(roleId string, menuCheckStrictly bool) []int {
	...
	var menuIds []int
	err := db.Db().Raw(sql).Scan(&menuIds).Error
	if err != nil {
		panic(R.ReturnFailMsg(err.Error()))
	}
	return menuIds
...
func FindMenuInfoById(menuId int) SysMenu {
	...
	var list SysMenu
	err := db.Db().Raw(sql, menuId).First(&list).Error
	if err != nil {
		panic(R.ReturnFailMsg(err.Error()))
	}
	return list
...
func hasChildByMenuId(menuId int) SysMenu {
	var menu SysMenu
	db.Db().Where("parent_id = ? ", menuId).First(&menu)
	return menu
}

func hasChildCountByMenuId(menuId int) int {
	var menuCount int
	err := db.Db().Where("parent_id = ? ", menuId).Scan(&menuCount).Error
	if err != nil {
		panic(R.ReturnFailMsg(err.Error()))
	}
	return menuCount
...
func checkMenuExistRole(menuId int) int {
	var menuCount int
	err := db.Db().Raw("select count(1) from sys_role_menu where menu_id = ?", menuId).Scan(&menuCount).Error
	if err != nil {
		panic(R.ReturnFailMsg(err.Error()))
	}
	return menuCount
...
func DeleteMenu(menuIds int) R.Result {
	...
	err := db.Db().Exec("delete from sys_menu where menu_id in (?) ", menuIds).Error
	if err == nil {
...
func checkMenuNameUnique(parentId int, menuName string) int {
	var menuCount int
	err := db.Db().Raw("select count(1) from sys_menu where menu_name = "+menuName+" and parent_id = "+strconv.Itoa(parentId), &menuCount).Error
	if err != nil {
		panic(R.ReturnFailMsg(err.Error()))
	}
	return menuCount
...
func AddMenu(sysMenu SysMenu) R.Result {
	...
	err := db.Db().Create(&sysMenu).Error
	if err != nil {
		panic(R.ReturnFailMsg(err.Error()))
	}
	return R.ReturnSuccess("操作成功")
...
func UpdateMenu(sysMenu SysMenu) R.Result {
	...
	err := db.Db().Updates(&sysMenu).Error
	if err != nil {
		panic(R.ReturnFailMsg(err.Error()))
	}
	return R.ReturnSuccess("操作成功")
