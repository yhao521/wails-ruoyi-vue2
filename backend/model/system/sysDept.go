...
func GetDeptInfo(deptId string) SysDept {
	var dept SysDept
	err := db.Db().Where("dept_id = ?", deptId).First(&dept).Error
	if err != nil {
...
func SaveDept(sysDept SysDept) R.Result {
	sysDept.DelFlag = "0"
	err := db.Db().Model(&sysDept).Create(&sysDept).Error
	if err != nil {
...
func UpDataDept(sysDept SysDept) R.Result {
	err := db.Db().Updates(&sysDept).Error
	if err != nil {
		return R.ReturnFailMsg(err.Error())
	}
	return R.ReturnSuccess("操作成功")
...
func DeleteDept(deptId string) R.Result {
	...
	//        }
	//是否有数据权限
	err := db.Db().Where("dept_id = ?", deptId).Delete(&SysDept{}).Error
	if err != nil {
		return R.ReturnFailMsg(err.Error())
	}
	return R.ReturnSuccess("操作成功")
...
func SelectDeptTreeList() []TreeSelect {
	...
	var depts []SysDept
	err := db.Db().Where("del_flag = '0'").Order("parent_id, order_num").Find(&depts).Error
	if err != nil {
		panic(R.ReturnFailMsg(err.Error()))
	}
