...
func IsExistUser(loginName string) bool {
	var user SysUser
	err := db.Db().Where("del_flag = 0 and user_name = ?", loginName).First(&user).Error
...
func IsExistUserByPhoneNumber(phonenumber string) bool {
	var user SysUser
	err := db.Db().Where("del_flag = 0 and phonenumber = ?", phonenumber).First(&user).Error
	if err != nil {
		return false
	}
	return true
...
func IsExistUserByEmail(email string) bool {
	var user SysUser
	err := db.Db().Where("del_flag = 0 and email = ?", email).First(&user).Error
	if err != nil {
		return false
	}
	return true
...
func FindUserByName(loginName string) SysUser {
	var user SysUser
	err := db.Db().Where("del_flag != 2 and user_name = ?", loginName).Find(&user).Error
	if err != nil {
...
func FindUserById(userId int) SysUser {
...
func FindUserPassById(userId int) SysUser {
	// userId := utils.GetInterfaceToInt(id)
	if userId < 1 {
		panic(R.ReturnFailMsg("获取用户信息失败"))
	}
...
func SaveUser(user SysUserParm) string {
	...
	err1 := db.Db().Model(&SysUser{}).Create(&u).Error
	if err1 != nil {
		result = "" + err1.Error()
	}
...
func EditProfileUserInfo(userId int, param Userparam) R.Result {
	...
	err2 := db.Db().Model(&SysUser{}).Where("user_id = ?", userId).Updates(data)
...
func DeleteUserById(userId []int) R.Result {
	err := db.Db().Exec("update sys_user set del_flag = '2' where user_id in (?) ", userId).Error
	if err != nil {
		panic(R.ReturnFailMsg(err.Error()))
	}
	return R.ReturnSuccess("操作成功")
...
func ChangeUserStatus(param SysUserParm) R.Result {
	...
	//等价于: UPDATE `foods` SET `price` = '35', `stock` = '0'  WHERE (user_id = '2')
	err2 := db.Db().Model(&SysUser{}).Where("user_id = ?", param.UserId).Updates(data)
	if err2.Error != nil {
		return R.ReturnFailMsg("修改失败")
	}
