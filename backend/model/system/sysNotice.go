...
func FindNoticeInfoById(noticeId int) SysNotice {
	var notice SysNotice
	err := db.Db().Where("notice_id = ?", noticeId).First(&notice).Error
	if err != nil {
...
func SaveNotice(sysNotice SysNotice) R.Result {
	err := db.Db().Model(&SysNotice{}).Create(&sysNotice).Error
	if err != nil {
...
func UploadNotice(sysNotice SysNotice) R.Result {
	err := db.Db().Updates(&sysNotice).Error
	if err != nil {
		return R.ReturnFailMsg(err.Error())
	}
	return R.ReturnSuccess("操作成功")
...
func DeleteNotice(noticeIds string) R.Result {
	...
	for i := 0; i < len(ids); i++ {
		id := ids[i]
		err := db.Db().Where("notice_id = ?", utils.GetInterfaceToInt(id)).Delete(&SysNotice{}).Error
		if err != nil {
			return R.ReturnFailMsg(err.Error())
		}
	}
