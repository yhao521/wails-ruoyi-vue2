...
func SelectRolePermissionByUserId(userId int) []SysRoles {
	...
	err := db.Db().Raw(sql).Scan(&roles).Error
	if err != nil {
...
func GetMenuPermission(user SysUser) []string {
	if IsAdmin(&user) {
		...
		var str string
		err := db.Db().Raw("select distinct m.perms "+
			"from sys_menu m "+
...
func SelectRolesByUserName(userName string) string {
	var roles []SysRoles
	var result = ""
	err := db.Db().Raw("select distinct r.* "+
		"from sys_role r "+
		"left join sys_user_role ur on ur.role_id = r.role_id "+
		"left join sys_user u on u.user_id = ur.user_id "+
		"left join sys_dept d on u.dept_id = d.dept_id "+
...
func DeleteRolesByUser(userId []int) {
	err := db.Db().Exec("delete from sys_user_role where user_id in (?)", userId).Error
	if err != nil {
...
func DeleteRolesById(roleId string, userId int) {
	...
	err := db.Db().Exec("update sys_role set del_flag = '2' where role_id in (" + roleId + ")").Error
	if err != nil {
		panic(R.ReturnFailMsg("删除权限关联用户失败"))
	}
}
...
func FindRoleInfoById(roleId string) SysRoles {
	var role SysRoles
	err := db.Db().Where("role_id = ?", roleId).First(&role).Error
	if err != nil {
		panic(R.ReturnFailMsg(err.Error()))
	}
	return role
...
func CheckRoleDataScope(roleId string) bool {
	...
	var count int64
	err := db.Db().Raw(sql).Count(&count).Error
	if err != nil {
		panic(R.ReturnFailMsg(err.Error()))
	}
	return count < 1
...
func checkRoleNameUnique(roleName string) SysRoles {
	...
	err := db.Db().Raw(sql).Scan(&role).Error
	if err != nil {
		panic(R.ReturnFailMsg(err.Error()))
	}
	return role
...
func countUserRoleByRoleId(roleId string) int {
	...
	var count int
	err := db.Db().Raw(sql).Scan(&count).Error
	if err != nil {
		panic(R.ReturnFailMsg(err.Error()))
	}
	return count
...
func SaveRole(roles SysRolesParam) R.Result {
	...
	roles.DataScope = "2"
	roles.DelFlag = "0"
	err := db.Db().Model(&SysRoles{}).Create(&roles).Error
	if err != nil {
...
func insertRoleMenu(roles SysRolesParam) {
	...
	err := db.Db().Model(&SysRoleMenu{}).Create(&roleMenu).Error
	if err != nil {
		panic(R.ReturnFailMsg(err.Error()))
	}
}
...
func insertRoleDept(roles SysRolesParam) {
	...
	err := db.Db().Model(&SysRoleDept{}).Create(&roleDept).Error
	if err != nil {
		panic(R.ReturnFailMsg(err.Error()))
	}
}
...
func PutDataScope(rolesParam SysRolesParam, userId int) R.Result {
	...
	checkRoleDataScope(strconv.Itoa(rolesParam.RoleId), userId)
	/*修改角色信息*/
	err := db.Db().Model(&SysRoles{}).Where("role_id", rolesParam.RoleId).Updates(&rolesParam).Error
	if err != nil {
		panic(R.ReturnFailMsg(err.Error()))
	}
	/*删除角色与部门关联*/
...
func ChangeRoleStatus(rolesParam SysRoles, userId int) R.Result {
	...
	/*校验角色是否有数据权限*/
	checkRoleDataScope(strconv.Itoa(rolesParam.RoleId), userId)
	err := db.Db().Updates(&rolesParam).Error
	if err != nil {
		return R.ReturnFailMsg(err.Error())
	}
	return R.ReturnSuccess("操作成功")
...
func GetRoleOptionSelect() R.Result {
	...
	sql += "order by r.role_sort"
	var rols []SysRoles
	err := db.Db().Raw(sql).Scan(&rols).Error
	if err != nil {
		panic(R.ReturnFailMsg(err.Error()))
	}
...
func CancelRole(userId int, roleId int) R.Result {
	err := db.Db().Exec("delete from sys_user_role where user_id = ? and role_id = ?", userId, roleId).Error
	if err != nil {
		panic(R.ReturnFailMsg(err.Error()))
	}
	return R.ReturnSuccess("操作成功")
...
func SelectRoleAll(roleId string, userIdStr string, userId int) R.Result {
	...
	for i := 0; i < len(userIds); i++ {
		id := userIds[i]
		userRoles = append(userRoles, SysUserRole{RoleId: utils.GetInterfaceToInt(roleId), UserId: utils.GetInterfaceToInt(id)})
	}
	err := db.Db().Model(&SysUserRole{}).Create(&userRoles).Error
	if err != nil {
		panic(R.ReturnFailMsg(err.Error()))
	}
	return R.ReturnSuccess("操作成功")
...
func GetDeptTreeRole(roleId string) []int {
	...
	var count []int
	err := db.Db().Raw(sql).Find(&count).Error
	if err != nil {
		panic(R.ReturnFailMsg(err.Error()))
	}
