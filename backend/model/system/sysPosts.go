...
func SelectUserPostGroup(userName string) string {
	var posts []SysPost
	var result = ""
	db.Db().Raw("select p.post_id, p.post_name, p.post_code "+
		"from sys_post p "+
...
func AddPostListByUser(postIds []int, userId int) {
	...
	err := db.Db().CreateInBatches(posts, len(posts)).Error
	if err != nil {
...
func DeletePostByUser(userIds []int) {
	err := db.Db().Exec("delete from sys_user_post where user_id in (?) ", userIds).Error
	if err != nil {
		panic(R.ReturnFailMsg("删除部门关联用户失败"))
	}
}

func FindPostInfoById(postId int) SysPost {
	var post SysPost
	err := db.Db().Where("post_id = ?", postId).First(&post).Error
	if err != nil {
...
func SavePost(post SysPost) R.Result {
	err := db.Db().Model(&SysPost{}).Create(&post).Error
	if err != nil {
...
func EditPost(sysPost SysPost) R.Result {
	err := db.Db().Updates(&sysPost).Error
	if err != nil {
		return R.ReturnFailMsg(err.Error())
	}
	return R.ReturnSuccess("操作成功")
...
func DeletePost(postIds string) R.Result {
	err := db.Db().Where("post_id in (?)", utils.Split(postIds)).Delete(&SysPost{}).Error
	if err != nil {
		return R.ReturnFailMsg(err.Error())
	}
	return R.ReturnSuccess("操作成功")
