...
// buildFileTemplate
function buildFileTemplate(scheme, child) {
  let labelPosition = ''
  if (scheme.labelPosition !== 'right') {
    labelPosition = `label-position="${scheme.labelPosition}"`
  }
  // const disabled = scheme.disabled ? `:disabled="${scheme.disabled}"` : ''
  let str = `<el-card shadow="hover" >
...
// import TableView from '@/components/template/TableView/index.vue'
const tags = {
  'el-button': el => {
    const {
      tag, disabled
  ...
  'yh-table-view': el => {
    ...
    // console.debug('TableView-tag', TableView)
    // console.debug('TableView-tag.dom', TableView.methods.getDom())
    return `<${tag} ${vModel} ${placeholder} ${height} ${branding}></${tag}>`
  },
  'yh-container': el => {
    const { tag, vModel, placeholder } = attrBuilder(el)
...
export function makeUpHtml(formConfig, type) {
  ...
  // 遍历渲染每个组件成html
  console.debug('makeUpHtml-formConfig.fields', formConfig.fields)
  formConfig.fields.forEach(el => {
    htmlList.push(layouts[el.__config__.layout](el))
  })
  console.debug('makeUpHtml-htmlList', htmlList)
  const htmlStr = htmlList.join('\n')
  // 将组件代码放进file noform 标签
  let temp = buildFileTemplate(formConfig, htmlStr)
  // 将组件代码放进form标签
  if (type === 'file') {
    temp = buildFormTemplate(formConfig, htmlStr, type)
  }
  // dialog标签包裹代码
  if (type === 'dialog') {
    temp = buildFormTemplate(formConfig, htmlStr, type)
