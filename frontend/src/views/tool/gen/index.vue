  ...
  <div class="app-container">
    ...
    <el-table v-loading="loading" :data="tableList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="序号" type="index" width="50" align="center">
        <template slot-scope="scope">
          <!-- <span>{{ (queryParams.pageNum - 1) * queryParams.pageSize + scope.$index + 1 }}-{{ scope.row.tableId }}</span> -->
    ...
    <el-dialog :title="preview.title" :visible.sync="preview.open" width="80%" top="5vh" append-to-body
      ...
      <el-tabs v-model="preview.activeName">
        ...
        <!-- <highlightjs :language="highlightedKey(key)" :code="highlsCode(value)"></highlightjs> -->
      </pre>
        </el-tab-pane>
      </el-tabs>
    </el-dialog>
...
export default {
  name: "Gen",
  components: { importTable, createTable, highlightjs: hljsVuePlugin.component },
  data() {
    return {
      // 遮罩层
      loading: true,
  ...
  methods: {
    ...
    getList() {
      ...
      this.queryParams = {
        ...
        listTable(this.addDateRange(this.queryParams, this.dateRange)).then(response => {
          ...
          this.total = response.total == undefined ? 0 : response.total;
          this.loading = false;
        }
        );
    },
    ...
    handleGenTable(row) {
      ...
      console.debug('handleGenTable', row)
      if (row.genType === "1") {
        genCode(row.tableName).then(response => {
          this.$modal.msgSuccess("成功生成到自定义路径：" + row.genPath);
        });
      } else {
        // this.$download.saveAsFile("ruoyi.zip", "*.go;*.zip").then(response => {
        // this.$download.selectDownloadPath().then(response => {
        //   console.debug(response)
        // this.$modal.msgSuccess("成功生成：" + response);

        genCode(tableNames).then(response => {
          this.$modal.msgSuccess("成功生成：");
        });
        // });
    ...
    highlightedCode(code, key) {
      ...
      const result = hljs.highlight(language || 'go', code || "", true);
      return result.value || '&nbsp;';
    },
    //code
    highlsCode(code) {
