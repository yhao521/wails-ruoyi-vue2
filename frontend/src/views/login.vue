...
<script>
import { getCodeImg, getCookies } from "@/api/login";
...
export default {
  ...
  methods: {
    ...
    getCookie() {
      getCookies().then(res => {
        console.debug('getCookies:', res)

        if (res.code === 200) {
          // sessionStorage.setItem('username', res.data.userName);
          // sessionStorage.setItem('password', decrypt(res.data.password));
          // sessionStorage.setItem('password', res.data.rememberMe);
          this.$store.dispatch('Login', this.loginForm).then(res => {
            if (res.code === 200) {
              this.$router.push({ path: this.redirect || '/' }).catch(err => { console.log(err) })
            } else {

              this.$router.push({ path: '/index' }).catch(err => { console.log(err) })
            }
      ...
      this.$refs.loginForm.validate(valid => {
        if (valid) {
          ...
          if (this.loginForm.rememberMe) {
            ...
            sessionStorage.setItem('password', this.loginForm.rememberMe);
          } else {
